...
function Spy2:RefreshList()
	local MainWindow = Spy2.MainWindow
	if not MainWindow:IsShown() then
		return
	end

	-- render order: every nearby player sorted by when they were first detected (their NearbyList
	-- timestamp, which never changes), with the name as a tiebreaker so the order is fully stable.
	-- active/inactive sets only the row opacity below, never the position, so a newly detected enemy
	-- always appends to the bottom and an active<->inactive change never reorders the list. each
	-- already-shown enemy keeps its own row, so in combat -- when the /targetexact macros are frozen --
	-- only the new bottom row is left un-targetable (red), not every row that would otherwise shift.
	local order = {}
	for player, first in pairs(Spy2.NearbyList) do
		table.insert(order, { player = player, time = first })
	end
	table.sort(order, function(a, b)
		if a.time == b.time then return a.player < b.player end
		return a.time < b.time
	end)

	-- the counter reports the full nearby list, which can exceed the displayed-row limit below
	if MainWindow.Counter then
		if #order > 0 then
			MainWindow.Counter:SetText(tostring(#order))
			MainWindow.Counter:Show()
		else
			MainWindow.Counter:Hide()
		end
	end

	-- the top N rows (N capped by the display limit) carry the enemies, packed from the top; every
	-- other row is blanked invisible and, out of combat, mouse-disabled so it passes clicks through.
	-- all rows stay shown (ManageBarsDisplayed), so an enemy first seen in combat -- when rows cannot
	-- be shown -- still lands on a row already on screen. its /targetexact macro can't be bound while
	-- combat-locked, so BindRow flags that row red until combat ends; this can't be avoided.
	local cap = Spy2.db.profile.ResizeSpyLimit
	if cap > Spy2.ButtonLimit then cap = Spy2.ButtonLimit end
	local shown = #order
	...
	for i = 1, Spy2.ButtonLimit do
		if i <= shown then
			...
			local playerData = Spy2.PlayerData[data.player]
			if playerData then
				if playerData.level then
					level = playerData.level
					if playerData.isGuess == true and tonumber(playerData.level) < Spy2.MaximumPlayerLevel then
						level = level.."+"
					end
				end
			...
			if Spy2.db.profile.DisplayListData == "1NameLevelClass" then
				description = level.." "
				if L[class] and type(L[class]) == "string" then
					description = description..L[class]
				end
			elseif Spy2.db.profile.DisplayListData == "2NameLevelGuild" then
				description = level.." "..guild
			elseif Spy2.db.profile.DisplayListData == "3NameLevelOnly" then
				description = level.." "
			elseif Spy2.db.profile.DisplayListData == "4NamePvPRank" then
				description = L["Rank"].." "..rank
			elseif Spy2.db.profile.DisplayListData == "5NameGuild" then
				description = guild
			end
			if Spy2.InactiveList[data.player] then
				-- faded rows are dimmed but kept clearly readable
				opacity = 0.7
			end

			Spy2:SetBar(i, data.player, description, 100, "Class", class, nil, opacity)
			Spy2.ButtonName[i] = data.player
			Spy2:BindRow(i, data.player)
			if not combat then Spy2.MainWindow.Rows[i]:EnableMouse(true) end
		else
			Spy2:BlankRow(i)
			Spy2.ButtonName[i] = nil
			-- EnableMouse is allowed only out of combat; an empty row left mouse-on briefly (a departure
			-- mid-fight) still shows nothing, and the next out-of-combat refresh disables it.
			if not combat and Spy2.MainWindow.Rows[i] then
				Spy2.MainWindow.Rows[i]:EnableMouse(false)
			end
		end
	end
	Spy2.ListAmountDisplayed = shown

	Spy2:AutomaticallyResize()
	Spy2:ManageBarsDisplayed()
end

function Spy2:BindRow(num, player)
	local row = Spy2.MainWindow.Rows[num]
	if not row then
		return
	end
	if not InCombatLockdown() then
		-- SetAttribute on a protected frame is only allowed out of combat
		row:SetAttribute("macrotext", "/targetexact "..Spy2:TargetName(player))
		row.boundName = player
		row.needsRebind = nil
	elseif row.boundName ~= player then
		-- the secure macro still points at the previously bound player; mark the row
		-- combat-unbound so the user knows left-clicking will not target correctly
		row.needsRebind = true
		row.StatusBar:SetStatusBarColor(0.8, 0.1, 0.1, 0.8)
	end
end

function Spy2:ManageExpirations()
	local expired = false
	local currentTime = time()
	for player in pairs(Spy2.ActiveList) do
		if (currentTime - Spy2.ActiveList[player]) > Spy2.ActiveTimeout then
			Spy2.InactiveList[player] = Spy2.ActiveList[player]
			Spy2.ActiveList[player] = nil
			expired = true
		end
	end
	if Spy2.db.profile.RemoveUndetected ~= "Never" then
		for player in pairs(Spy2.InactiveList) do
			if (currentTime - Spy2.InactiveList[player]) > Spy2.InactiveTimeout then
				Spy2.InactiveList[player] = nil
				Spy2.NearbyList[player] = nil
				expired = true
			end
		end
	end
...
function Spy2:ClearList()
	-- the rows are secure buttons: they can't be hidden nor the window resized while combat-locked, so
	-- a clear can't take effect in combat. defer it to PLAYER_REGEN_ENABLED (LeftCombatEvent) so a
	-- reset pressed in combat clears the moment combat ends.
	if InCombatLockdown() then
		Spy2.ClearPending = true
		return
	end
	Spy2.ClearPending = nil
	Spy2.NearbyList = {}
	Spy2.ActiveList = {}
	Spy2.InactiveList = {}
	Spy2.ListAmountDisplayed = 0
	-- with the list empty the window resizes to a hairline; it stays shown and has nothing on it
	Spy2:RefreshList()
end

function Spy2:AddPlayerData(name, class, level, race, guild, faction, isEnemy, isGuess, rank)
	local info = {}
	...
	info.rank = rank
	Spy2.PlayerData[name] = info
	return Spy2.PlayerData[name]
end

function Spy2:UpdatePlayerData(name, class, level, race, guild, faction, isEnemy, isGuess, rank)
	local detected = true
	local playerData = Spy2.PlayerData[name]
	if not playerData then
		playerData = Spy2:AddPlayerData(name, class, level, race, guild, faction, isEnemy, isGuess, rank)
	else
	...
	if playerData then
		...
		if not Spy2.ActiveList[name] then
...
function Spy2:UpdatePlayerStatus(name, class, level, race, guild, faction, isEnemy, isGuess, rank)
	local playerData = Spy2.PlayerData[name]
	if not playerData then
		playerData = Spy2:AddPlayerData(name, class, level, race, guild, faction, isEnemy, isGuess, rank)
	else
...
function Spy2:RemovePlayerData(name)
	Spy2.PlayerData[name] = nil
end

-- kind is "stealth" or "prowl"; both raise the same stealth warning, routed to the error frame or a
-- popup alert per DisplayWarnings, with an optional sound.
function Spy2:AlertStealthOrProwl(kind, player)
	if not Spy2.db.profile.WarnOnStealth then return end
	if Spy2.db.profile.DisplayWarnings == "ErrorFrame" then
		local text = Spy2.db.profile.Colors.Warning["Warning Text"]
		UIErrorsFrame:AddMessage(L["StealthWarning"]..player, text.r, text.g, text.b, 1.0, UIERRORS_HOLD_TIME)
	else
		Spy2:ShowAlert(kind, player)
	end
	if Spy2.db.profile.EnableSound then
		PlaySound(SOUNDKIT.RAID_WARNING, Spy2.db.profile.SoundChannel)
	end
end

function Spy2:AlertStealthPlayer(player) Spy2:AlertStealthOrProwl("stealth", player) end
function Spy2:AlertProwlPlayer(player) Spy2:AlertStealthOrProwl("prowl", player) end

function Spy2:ButtonClicked(self, button)
	if button == "RightButton" then
		if Spy2.db.profile.RightClickClear then
			Spy2:ClearList()
		end
	end
	-- LeftButton targets via the secure /targetexact macro, bound eagerly in BindRow.
end

function Spy2:ParseMinimapTooltip(tooltip)
	local newTooltip = ""
	local newLine = false
	for text in string.gmatch(tooltip, "[^\n]*") do
		...
		if string.len(text) > 0 then
			...
			local playerData = Spy2.PlayerData[name]
			-- the minimap tooltip shows a bare name; PlayerData keys players "Name-Realm", so resolve
			-- to the realm-qualified key: an existing entry matched on the character-name prefix, else
			-- the player's own realm (correct for the same-realm players the minimap mostly shows).
			local key = Spy2:CanonicalName(name)
			if not playerData then
				for index in pairs(Spy2.PlayerData) do
					...
					if realmSeparator and realmSeparator > 1 and strsub(index, 1, realmSeparator - 1) == strsub(name, 1, realmSeparator - 1) then
						playerData = Spy2.PlayerData[index]
						key = index
						break
					end
				end
			...
			if playerData and playerData.isEnemy then
				...
				if not Spy2.InInstance then
					local detected = Spy2:UpdatePlayerData(key, nil, nil, nil, nil, nil, true, nil, nil)
					if detected and Spy2.db.profile.MinimapDetection then
						Spy2:AddDetected(key, time(), false)
					end
				end
			else
				if (newTooltip == "") then
...
function Spy2:ParseUnitAbility(analyseSpell, event, player, class, race, spellId, spellName)
	local learnt = false
	if player then
		local level = nil
		local isEnemy = true
		...
		local playerData = Spy2.PlayerData[player]
		...
		if analyseSpell then
			...
			if abilityType == "SWING" or abilityType == "SPELL" or abilityType == "RANGE" then
				local ability = Spy2_AbilityList[spellId]
				if ability then
					if class == nil then
...
--					print(spellId, " - ", spellName)
				...
				if class and race and level == Spy2.MaximumPlayerLevel then
					isGuess = false
					learnt = true
				end
			end
		...
		Spy2:UpdatePlayerData(player, class, level, race, nil, nil, isEnemy, isGuess)
		return learnt, playerData
	end
	return learnt, nil
end

function Spy2:AddDetected(player, timestamp, learnt, source)
	if Spy2.db.profile.StopAlertsOnTaxi then
		if not UnitOnTaxi("player") then 
			Spy2:AddDetectedToLists(player, timestamp, learnt, source)
		end
	else
		Spy2:AddDetectedToLists(player, timestamp, learnt, source)
	end
...
function Spy2:AddDetectedToLists(player, timestamp, learnt, source)
	if not Spy2.NearbyList[player] then
		Spy2.NearbyList[player] = timestamp
		Spy2.ActiveList[player] = timestamp
		Spy2.InactiveList[player] = nil
		Spy2:RefreshList()
	elseif not Spy2.ActiveList[player] then
		Spy2.ActiveList[player] = timestamp
		Spy2.InactiveList[player] = nil
		Spy2:RefreshList()
	else
...
-- /spy2 test: fills the Nearby list with one fake enemy of every class plus an unknown-class
-- guess, so the window can be checked without real players around. The fakes age out through
-- the normal expiration timer, or remove them immediately with /spy2 clear.
function Spy2:AddTestData()
	local fakes = {
		{ name = "Bigslam",     class = "WARRIOR", level = 60, race = "Orc" },
		{ name = "Holyfist",    class = "PALADIN", level = 60, race = "Human" },
		{ name = "Beastmaster", class = "HUNTER",  level = 59, race = "Dwarf" },
		{ name = "Sneakattack", class = "ROGUE",   level = 60, race = "Night Elf" },
		{ name = "Mendwell",    class = "PRIEST",  level = 58, race = "Undead" },
		{ name = "Totemic",     class = "SHAMAN",  level = 60, race = "Tauren" },
		{ name = "Frostfire",   class = "MAGE",    level = 57, race = "Gnome" },
		{ name = "Felhunter",   class = "WARLOCK", level = 60, race = "Troll" },
		{ name = "Barkskin",    class = "DRUID",   level = 56, race = "Night Elf" },
		{ name = "Mystery",     class = "UNKNOWN", level = 58, race = "Unknown", guess = true },
	}
	local now = time()
	for i, f in ipairs(fakes) do
		Spy2:UpdatePlayerData(f.name, f.class, f.level, f.race, "Test Guild", Spy2.EnemyFactionName, true, f.guess == true, i % 6)
		Spy2:AddDetectedToLists(f.name, now, true, "test")
	end
	Spy2:Print("Added "..#fakes.." test players to the Nearby list. Use /spy2 clear to remove them.")
end
